...
class PaidController extends BaseController
{
    ...
    {
        $data = []; $data = $this->paidServices->create_wallet($request);
    ...
    public function paid( $id)
{
    if (Auth::user()->role_name == 'client') {
        ...
        if (!$wallet || !$reservation) {
            return response(['message' => 'Wallet or Reservation not found'], 404);
        }

        if ($payment->amount > $wallet->balance) {
            return response([
                'wallet_balance' => $wallet->balance,
                'message' => 'The reservation cost is higher than your wallet balance. If the balance is not filled sufficiently and payment is not made within a week, your reservation will be deleted.'
            ], 400);
        } else {
